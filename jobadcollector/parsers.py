...
class DuunitoriParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if (tag == "section" and ('class', 'setion--secondary') in attrs):
            self.job_list = 0
        elif (self.job_list == 1 and tag == "a" and 
              ('class', 'jobentry--item') in attrs):
            self.job = 1
            for entry in attrs:
                if entry[0] == "href":
                    self.url = "http://www.duunitori.fi/" + entry[1].strip()
                    self.id = entry[1].strip().split("-")[-1]
        elif (self.job == 1 and tag == "h3" and 
              ('itemprop', 'title') in attrs):
            self.title_data = 1

    def handle_endtag(self, tag):
        if (self.job_list == 1 and self.job == 1 and self.title_data == 1
            and tag == "h3"):
            self.title_data = 0
        elif(self.job_list == 1 and self.job == 1 and self.title_data == 0 and
             tag == "a"):
            self.save_job_ad(self.id, self.title, self.url, self.additional)
            self.job = 0
            self.title = ""
    ...
    def handle_data(self, data):
        if (self.job_list == 1 and self.job == 1 and self.title_data == 1):
            self.title = self.title + " " + data.strip()
        elif(self.job_list == 1 and self.job == 1 and self.title_added == 0):
            self.additional = self.additional + " " + data.strip()
